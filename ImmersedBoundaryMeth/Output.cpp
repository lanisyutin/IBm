...
void OutputPressure(MatrixXd data, int n, double output_step, map<int, Circle*> iList, Grid grid){
	...
	for (auto& solid : iList){
		...
		output << "GEOMETRY ZN = " << n / output_step + 1 << ", X=" << solid.second->Bound[0][0] << ", Y=" << solid.second->Bound[1][0] << ", T=LINE, LT = 0.3, CS=GRID" << endl;
		output << "1" << endl << grid.NF + 1 << endl;
		output << 0 << " " << 0 << endl;
		for (int i = 1; i < grid.NF; ++i){

			output << solid.second->Bound[0][i] - solid.second->Bound[0][0] << " " << solid.second->Bound[1][i] - solid.second->Bound[1][0] << endl;
